...
function rabbit_encrypt() {
    var encrypted = CryptoJS.Rabbit.encrypt(rabbit_text.value, rabbit_key.value);
    rabbit_res.innerHTML = encrypted;
}
function rabbit_decrypt() {
    var decrypted = CryptoJS.Rabbit.decrypt(rabbit_text.value, rabbit_key.value).toString(CryptoJS.enc.Utf8);
    rabbit_res.innerHTML = decrypted;
}